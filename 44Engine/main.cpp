    ...
    _In_ int       nCmdShow)
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            // 프로그램이 한번 메모리에 올려진 것을 공유하므로 사용공간 자체가 정적라이브러리보다 적다.
            // 프로그램 중간에 동적 라이브러리를 업데이트하거나 변경해도 원본프로그램에는 문제가 없다.
            // 잘못 업데이트하면 중간에 프로그램이 죽어버린다.
            // 
            // 정적 라이브러리 : 프로그램을 컴파일하는 과정에서 포함시키는 라이브러리
            // 윈도우 -> .lib 리눅스 .a
            // 단점 : 동적 라이브러리에 비해서 각각 할당해줘야하기 때문에 용량을 많이 차지한다.
            // 해당 정적 라이브러리를 참조하는 프로그램이 많아질수록 메모리 사용량이 배로 늘어난다.
            // 재배포해줄시에 프로그램을 종료하고 새로 빌드한 후 배포해주어야 한다.
            // 
            // 최대큰 단점 : cpp 파일이 많아져서 링킹이 되게 오래 걸린다.
            // 장점 : 컴파일시 프로그램에 적재되기때문에 이식성이 좋다.
            // 런타임시에 동적 라이브러리 파일처럼 다른 메모리에 참조할 필요가 없기 때문에
            // 속도에서 장점이 있다.
